...
- Distance to nearest competitor is straight-line, so it can in principle cross water.
  Competitors more than 8 km from any mainland locality are dropped, which removes the
  Orkney, Shetland and Western Isles points; without that filter Wick and Thurso would be
  measured to a jeweller in Orkney, a ferry away. A handful of points on Bute, Mull and Skye
  remain because those islands fall in mainland council areas; the only one that is any
  town's nearest competitor is on Bute, 12.6 km from Largs, which does not change Largs'
  rank.
